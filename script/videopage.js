...
function executeOtherCode(videoId) {
  async function getVideoDetails(videoId) {
    try {
      ...
      const apiKey = "AIzaSyAGturmIunkaZW-RBQbqc1w0aASOhuSSYY";
      ...
      getChannelInfo(channelId, apiKey)
        .then((channelData) => {
          ...
          let upLoadDate = convertYouTubeAPIDate(
          ...
          const commentChannelLogo = document.querySelector(
            ".comment-channel-logo img"
          );
          ...
          let subsCount = formatCount(
            channelInformation.items[0].statistics.subscriberCount
          );
          channelSubs.innerHTML = `${subsCount} SUBSCRIBERS`;

          const channelComments = document.querySelector(".comments-count");
          let commentCount = formatCount(
            videoDetails.items[0].statistics.commentCount
          );
          channelComments.innerHTML = `${commentCount} Comments`;

          const LikeCount = formatCount(
  ...
  videoList.forEach((singleVideo) => {
    ...
    function appendUi(channelDetails, vdetails, videoId) {
      ...
      const viewCount = formatViewCount(
        vdetails.items[0]?.statistics.viewCount
      );
